...
final featureTogglesProvider =
    StateNotifierProvider<FeatureToggleNotifier, AsyncValue<List<FeatureToggle>>>(
  (ref) {
...
class FeatureToggleNotifier
  ...
  FeatureToggleNotifier(this._repository)
  ...
  Future<void> _loadToggles() async {
    ...
    debugPrint('[FeatureToggleNotifier] Starting load, has cached data: $hasCached');

    if (hasCached) {
      // CACHE HIT: Load immediately (non-blocking)
      final cached = await _repository.loadFromCache();
      debugPrint('[FeatureToggleNotifier] Cache hit, loaded ${cached.length} toggles');
      ...
      try {
        ...
        debugPrint('[FeatureToggleNotifier] API fetch completed with ${toggles.length} toggles');
  ...
  Future<void> _refreshInBackground() async {
    ...
    try {
      ...
      if (fresh.isNotEmpty) {
        debugPrint('[FeatureToggleNotifier] Background refresh got ${fresh.length} toggles, updating state');
        state = AsyncValue.data(fresh);
      } else {
        debugPrint('[FeatureToggleNotifier] Background refresh returned empty, keeping cached data');
      }
    } catch (e) {
      debugPrint('[FeatureToggleNotifier] Background refresh error: $e, keeping cached data');
    }
  }
  ...
  Future<void> refresh() async {
    ...
    try {
      final fresh = await _repository.refreshFromApi();
      if (fresh.isNotEmpty) {
        debugPrint('[FeatureToggleNotifier] Manual refresh got ${fresh.length} toggles');
        state = AsyncValue.data(fresh);
      } else {
        debugPrint('[FeatureToggleNotifier] Manual refresh returned empty, keeping current ${current.length} toggles');
...
///
/// Returns false for unknown toggles or during errors (safe default)
final featureEnabledProvider = Provider.family<bool, String>((ref, key) {
  ...
  return togglesAsync.when(
    ...
    loading: () => false, // Safe default during loading
    error: (_, __) => false, // Safe default on error
  );
});
